...
const sizes = { 'extra-small': 'min-h-(--inlay-button-xs-height) px-2 py-1 text-xs', small: 'min-h-(--inlay-button-sm-height) px-2.5 py-1 text-sm', medium: 'min-h-(--inlay-button-height) px-3 py-1.5 text-sm', large: 'min-h-(--inlay-button-lg-height) px-3.5 py-2 text-sm' }
...
<template>
  ...
  v-else-if="downloadHref && rows.length === 0"
    ...
    :class="['relative inline-flex items-center justify-center gap-1.5 border font-semibold focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-(--inlay-accent)', refused ? 'pointer-events-none opacity-50' : '', groupPosition ? groupPosition === 'single' ? 'rounded-(--inlay-radius)' : groupPosition === 'first' ? 'rounded-l-(--inlay-radius) rounded-r-none' : groupPosition === 'last' ? 'rounded-l-none rounded-r-(--inlay-radius)' : 'rounded-none' : style === 'icon-button' ? 'rounded-full p-0' : style === 'link' ? 'rounded-sm shadow-none underline-offset-4 hover:underline' : style === 'badge' ? 'rounded-full shadow-none' : 'rounded-(--inlay-radius) shadow-xs', size, tone]"
  ...
  >
    ...
    <span :class="style === 'icon-button' ? 'sr-only' : undefined">{{ action.label }}</span>
    <NamedIcon v-if="action.icon && action.iconPosition === 'after'" fallback="◆" :name="action.icon" :registries="registries" :renderers="renderers" />
    <span v-if="action.badge !== null && action.badge !== undefined" :class="[style === 'icon-button' ? 'absolute -right-1 -top-1 min-w-4' : 'ml-1', 'rounded-full border px-1.5 text-xs font-semibold', badgeTone]" :data-color="action.badgeColor ?? 'default'" data-slot="action-badge">{{ action.badge }}</span>
  </a>
  <button
    v-else
    :aria-disabled="refused || undefined"
    ...
    :aria-label="style === 'icon-button' ? action.label : undefined"
    :class="['relative inline-flex items-center justify-center gap-1.5 border font-semibold focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-(--inlay-accent) disabled:cursor-not-allowed disabled:opacity-50', groupPosition ? groupPosition === 'single' ? 'rounded-(--inlay-radius)' : groupPosition === 'first' ? 'rounded-l-(--inlay-radius) rounded-r-none' : groupPosition === 'last' ? 'rounded-l-none rounded-r-(--inlay-radius)' : 'rounded-none' : style === 'icon-button' ? 'rounded-full p-0' : style === 'link' ? 'rounded-sm shadow-none underline-offset-4 hover:underline' : style === 'badge' ? 'rounded-full shadow-none' : 'rounded-(--inlay-radius) shadow-xs', size, tone]"
    :data-color="action.color ?? 'default'"
    :data-outlined="action.outlined ? 'true' : undefined"
    :data-size="action.size ?? 'medium'"
    :data-trigger-style="style"
